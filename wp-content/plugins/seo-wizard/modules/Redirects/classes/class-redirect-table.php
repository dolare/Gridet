...
/**
 * Class WSW_Redirect_Table
 */
class WSW_Redirect_Table extends WP_List_Table {
	...
	public function __construct( $type ) {
		...
		$this->type             = $type;
		$class_name             = 'WSW_' . strtoupper( $this->type ) . '_Redirect_Manager';
	...
	public function get_columns() {
		$columns = array(
			...
			'old'  => $this->type,
			'new'  => __( 'New URL', 'seo-wizard' ),
			'type' => _x( 'Type', 'noun', 'seo-wizard' )
		);
	 ...
	 * Set the items as the WSW_Redirect_Table items variable.
	 */
	...
	public function column_old( $item ) {
		$actions = array(
			'edit'  => '<a href="javascript:;">' . __( 'Edit', 'seo-wizard' ) . '</a>',
			'trash' => '<a href="javascript:;" >' . __( 'Delete', 'seo-wizard' ) . '</a>',
		);
	...
	public function column_cb( $item ) {
		return sprintf(
			'<input type="checkbox" name="wsw_redirects_bulk_delete[]" value="%s" />', $item['old']
		);
	}
	...
	private function handle_bulk_action() {
		if ( isset( $_POST['action'] ) || isset( $_POST['action2'] ) ) {
			if ( 'delete' == $_POST['action'] || 'delete' == $_POST['action2'] ) {
				if ( isset( $_POST['wsw_redirects_bulk_delete'] ) && is_array( $_POST['wsw_redirects_bulk_delete'] ) && count( $_POST['wsw_redirects_bulk_delete'] ) > 0 ) {
					$this->redirect_manager->delete_redirect( $_POST['wsw_redirects_bulk_delete'] );
				}
			}
		}
	}
